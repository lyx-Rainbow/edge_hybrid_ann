...
methods = ["iRangeGraph", "DIGRA", "UNIFY", "Wow", "RecencyBucket"]

# 每个数据集下，每个算法对应的数值（Index Time, 单位: 秒）
# 顺序需与 methods 一一对应
data = {
   "longMemEval":  [221.73, 257.06, 124.94, 327.65, 349.21],
    "FNSPID": [247.73, 236.14, 116.32, 273.65, 312.64],
    "GIST1M": [129.85, 58.69, 78.84, 129.37, 230.64],
    "Paper":  [277.15, 209.76, 134.16, 399.06, 495.18],
}
...
colors = ["#3d85c6", "#8e7cc3", "#6aa84f", "#f6b26b", "#e74c3c"]
hatches = ["xx", "//", "\\\\", "xx", "..", "//"]
...
for group_idx, group in enumerate(dataset_groups, start=1):
    ...
    for ax, ds in zip(axes, group):
        ...
        ax.set_ylabel("Insert Throughput (/s)", fontsize=26)
        ax.tick_params(axis="y", labelsize=22)

    plt.tight_layout()
    out_name = f"grouped_bar_chart_{'_'.join(group)}.png"
...
plt.axis("off")
fig_leg.savefig("grouped_bar_chart_legend.png", dpi=200, bbox_inches="tight", pad_inches=0.05)
