#!/usr/bin/env python3
"""
fig_cp_qps_recall_100k.py — Complex Predicate QPS vs Recall for 100K benchmark.

One subplot per dataset. Each method shown as a Pareto frontier curve
of CP aggregated results (6 representative filters).

Usage:
    python 5_Plot/fig_cp_qps_recall_100k.py
"""
import json, os, sys
...
def load_cp_points(method_key, dataset):
    """Load CP points from sweep JSON. Returns list of (recall, qps) or None."""
    ...
    for r in data.get("sweep_results", []):
        ...
        if cp and cp.get("qps", 0) > 0:  # include all CP points (Curator may have low recall)
            points.append((cp["avg_recall"], cp["qps"]))
    if not points:
        return None
    points.sort()
    return points
...
def plot_all():
    ...
    for ax_idx, ds in enumerate(DATASETS_100K):
        ...
        ax.set_xlabel(f"CP Recall@{K}", fontsize=14)
        ax.set_ylabel("CP QPS", fontsize=14)
        ax.set_yscale("log")

        ds_recalls = []
        for method_key in ["curator", "diskivf", "spann", "prefilter"]:
            pts = load_cp_points(method_key, ds)
            if pts:
                ds_recalls.extend(p[0] for p in pts)
        ...
        n_pts = {}
        for method_key in ["curator", "diskivf", "spann", "prefilter"]:
            points = load_cp_points(method_key, ds)
            if not points:
                continue
    ...
    out_path = OUTPUT_DIR / "fig_cp_qps_recall_100k.png"
