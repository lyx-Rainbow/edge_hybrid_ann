...
size_t MultiTenantIndexIVFFlat::get_total_memory_bytes() const {
    size_t total = 0;

    // Quantizer (IndexFlat centroids)
    if (quantizer) {
        total += quantizer->ntotal * d * sizeof(float);
    }

    // Inverted lists: codes (raw float vectors) + ids
    if (invlists) {
        size_t n = invlists->compute_ntotal();
        total += n * code_size;  // raw float vectors as codes
        total += n * sizeof(idx_t);  // vector IDs
        // Per-list overhead
        total += nlist * sizeof(size_t);  // list sizes array
    }

    // Access map
    for (const auto& [xid, tid_set] : access_map) {
        total += sizeof(xid) + sizeof(tid_t) * tid_set.size();
    }
