...
size_t MultiTenantIndexIVFFlatSep::get_total_memory_bytes() const {
    size_t total = 0;

    // Per-tenant indexes
    for (const auto& [tid, idx] : indexes) {
        // Each IndexIVFFlat has its own InvertedLists + quantizer reference
        if (idx && idx->invlists) {
            total += idx->invlists->compute_ntotal() * (d * sizeof(float) + sizeof(idx_t));
        }
        if (idx && idx->quantizer) {
            total += idx->quantizer->ntotal * d * sizeof(float);
        }
    }

    // Per-tenant quantizers (IndexFlatL2 centroids)
    for (const auto& [tid, q] : quantizers) {
        if (q) {
            total += q->ntotal * d * sizeof(float);
        }
    }

    // Access control data structures
    for (const auto& [xid, tid_set] : xid_to_tids) {
        total += sizeof(xid) + tid_set.size() * sizeof(tid_t);
    }
    for (const auto& [xid, tid] : xid_to_owner) {
        total += sizeof(xid) + sizeof(tid_t);
    }
