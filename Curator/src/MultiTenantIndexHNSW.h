...
class PermissionCheck : public hnswlib::BaseFilterFunctor {
   ...
   public:
    ...
    bool operator()(hnswlib::labeltype label_id) {
        auto it = access_map.find(label_id);
        if (it == access_map.end()) return false;
        return it->second.find(tenant) != it->second.end();
    }
};
...
class ComplexPredicateCheck : public hnswlib::BaseFilterFunctor {
   ...
   public:
    ...
    bool operator()(hnswlib::labeltype label_id) {
        auto it = access_map.find(label_id);
        if (it == access_map.end()) return false;
        return complex_predicate::evaluate_formula(tokens, it->second);
    }
};
