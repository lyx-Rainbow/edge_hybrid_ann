"""
DiskIVF-PostFiltering index: k-means partitions on disk, post-filtered search.

Build:   k-means clustering → each partition saved as file on disk.
...
Usage:
    python DiskIVF-PostFiltering/run_diskivf.py --dataset yfcc100m
    python DiskIVF-PostFiltering/run_diskivf.py --config config/DiskIVF-PostFiltering/diskivf_yfcc100m.json
"""
...
class DiskIVFIndex:
    """Disk-backed IVF index with post-filtering."""
    ...
    def __init__(
        ...
        self.cluster_sizes = None   # [nlist]
        self._built = False

    def build(self, train_vecs: np.ndarray, train_mds: list):
        """K-means clustering, save partitions to disk."""
        import faiss
        ...
        if self.disk_dir is None:
            self.disk_dir = "4_Results/DiskIVF-PostFiltering/disk_data"
    ...
    def _get_nearest_clusters(self, x: np.ndarray) -> np.ndarray:
        """Return cluster IDs of nearest nprobe clusters."""
    ...
    def _load_cluster(self, cid: int):
        """Load vectors, indices, and mds for a cluster."""
        disk = Path(self.disk_dir)
    ...
    def get_index_memory_bytes(self) -> int:
        """In-memory components: centroids, cluster sizes."""
        mem = 0
    ...
    def get_disk_bytes(self) -> int:
        """Total disk usage of cluster files."""
        total = 0
...
# ---------------------------------------------------------------------------
# Shared helpers (identical to Pre-Filtering runner)
# ---------------------------------------------------------------------------
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="4_Results/DiskIVF-PostFiltering")
    parser.add_argument("--disk_dir", type=str, default="4_Results/DiskIVF-PostFiltering/disk_data")
    ...
    # ---- Load ----
    print(f"\n{'='*60}\nDiskIVF-PostFiltering Benchmark: {dataset}\n{'='*60}")
    ...
    # ---- Build ----
    print(f"\n{'='*60}\nBuilding index\n{'='*60}")
    index = DiskIVFIndex(nlist=cfg["nlist"], nprobe=cfg["nprobe"],
    ...
    print(f"  CP recall@{k}: {np.mean(cp_all_recalls):.4f}, "
          f"latency: {np.mean(cp_all_lats):.2f}ms, total: {total_cp_time:.1f}s")
    ...
    out_path = output_dir / f"diskivf_{dataset}.json"
