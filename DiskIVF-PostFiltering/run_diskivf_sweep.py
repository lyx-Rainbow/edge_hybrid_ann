"""
DiskIVF-PostFiltering parameter sweep (single-label queries, CP placeholder).

Sweeps nprobe (clusters loaded per query). Higher nprobe = better recall, slower.

Usage:
    python DiskIVF-PostFiltering/run_diskivf_sweep.py --dataset yfcc100m_small \
        --sweep config/DiskIVF-PostFiltering/sweep.json
"""
...
from run_diskivf import (  # noqa: E402
    DiskIVFIndex, compute_recall, load_data,
...
def main():
    ...
    parser.add_argument("--disk_dir", type=str, default="4_Results/DiskIVF-PostFiltering/disk_data_sweep")
    ...
    print(f"\n{'='*60}\nDiskIVF-PostFiltering Sweep: {dataset}\n{'='*60}")
    ...
    # Build once (use max nprobe from sweep for nprobe, then change per combo)
    max_nprobe = max(sweep_cfg.get("nprobe", [cfg.get("nprobe", 16)]))
    index = DiskIVFIndex(
        nlist=cfg.get("nlist", 64),
        nprobe=max_nprobe,
    ...
    for i, combo in enumerate(combos):
        ...
        # per_bucket already included ✓ (inline benchmark loop)
    ...
    out = {
        "dataset": dataset, "index": "DiskIVF-PostFiltering", "k": k,
        "nlist": cfg.get("nlist", 64),
