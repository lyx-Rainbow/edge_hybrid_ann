...
SPANNConfig load_config_from_json(const std::string& path) {
    ...
    get_int("bkt_kmeans_k", cfg.bkt_kmeans_k);
...
void print_usage() {
    printf("Usage: spann_pf <bench|search> [options]\n");
    printf("\nbench options:\n");
    ...
    printf("  --index_dir PATH       SPTAG index directory (required)\n");
    printf("  --metadata_path PATH   Label metadata file (default: <index_dir>/metadata.bin)\n");
    printf("  --queries PATH         Query vectors .npy [Q, d] float32 (required)\n");
    printf("  --k K                  Results per query (default: 10)\n");
    printf("  --max_check N          Override head index max check (search sweep)\n");
    printf("  --overfetch_factor N   Override overfetch factor (search sweep)\n");
...
int main(int argc, char** argv) {
    ...
    if (is_search && index_dir.empty()) {
        fprintf(stderr, "Error: --index_dir is required for search mode\n");
        print_usage();
        return 1;
    }
    ...
    printf("=== SPANN-PostFiltering %s mode ===\n", is_search ? "search" : "bench");
    ...
    // Apply CLI overrides for search-time parameters
    if (max_check_override > 0) cfg.max_check = max_check_override;
    if (overfetch_override > 0) cfg.overfetch_factor = overfetch_override;
    ...
    if (is_search) {
        // ── Search mode: load pre-built index ──
        cfg.index_dir = index_dir;
        if (metadata_path.empty()) {
            metadata_path = index_dir + "/metadata.bin";
        }
        ...
        index.load(index_dir, metadata_path);
        auto t_load_end = std::chrono::high_resolution_clock::now();
        build_time_s = std::chrono::duration<double>(t_load_end - t_load_start).count();

        // Re-apply config overrides after load (search-time params only)
        if (max_check_override > 0) cfg.max_check = max_check_override;
        if (overfetch_override > 0) cfg.overfetch_factor = overfetch_override;
        index.set_search_parameters(cfg.max_check, cfg.overfetch_factor);
