...
extern "C" __declspec(dllexport) winrt::SPTAG::LogLevel SPTAG_GetLoggerLevel()
{
    return winrt::SPTAG::LogLevel::Empty;
}
...
JsonObject CreateJsonObject(bool v)
{
    auto js = JsonObject{};
    js.Insert(L"value", JsonValue::CreateBooleanValue(v));
    return js;
}
JsonObject CreateJsonObject(std::wstring_view v)
{
    auto js = JsonObject{};
    js.Insert(L"value", JsonValue::CreateStringValue(v));
    return js;
}
template <size_t N> JsonObject CreateJsonObject(const wchar_t (&v)[N])
{
    return CreateJsonObject(winrt::hstring(v));
}
JsonObject CreateJsonObject(double v)
{
    auto js = JsonObject{};
    js.Insert(L"value", JsonValue::CreateNumberValue(v));
    return js;
}
JsonObject CreateJsonObject(nullptr_t)
{
    auto js = JsonObject{};
    js.Insert(L"value", JsonValue::CreateNullValue());
    return js;
}

template <typename T> winrt::array_view<uint8_t> Serialize(const T &value)
{
...
int main()
{
    ...
    auto file =
        folder.CreateFileAsync(L"vector_index", winrt::Windows::Storage::CreationCollisionOption::ReplaceExisting)
            .get();
    idx.Save(file);
